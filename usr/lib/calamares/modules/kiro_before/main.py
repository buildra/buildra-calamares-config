...
def move_mkinitcpio_preset():
    target_root = libcalamares.globalstorage.value("rootMountPoint")
    src = os.path.join(target_root, "etc/mkinitcpio.d/buildra")
    dst = os.path.join(target_root, "etc/mkinitcpio.d/linux.preset")

    libcalamares.utils.debug("-> Moving buildra preset to linux.preset in target...")
    try:
        os.replace(src, dst)
...
def run():
    ...
    libcalamares.utils.debug("Start buildra_before")
    libcalamares.utils.debug("#################################\n")
    ...
    for step_func in [
        ...
        if error:
            return error
    libcalamares.utils.debug("#################################")
    libcalamares.utils.debug("End buildra_before")
    libcalamares.utils.debug("#################################\n")
    return None
